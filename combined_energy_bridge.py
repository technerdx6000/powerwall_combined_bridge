...
from powerwall_v1r_direct import fetch_artifact, make_seeded_client
from pypowerwall.tedapi.tedapi_v1r import TEDAPIv1r


DEFAULT_CONFIG_PATH = "bridge_config.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", default=DEFAULT_CONFIG_PATH, help="Path to bridge configuration JSON")
    parser.add_argument("--once", action="store_true", help="Print a single snapshot JSON and exit")
    parser.add_argument("--host", default="127.0.0.1", help="HTTP bind host for serve mode")
...
def combine_snapshot(config: dict[str, Any]) -> dict[str, Any]:
    ...
    for site_name, site_config in config.get("powerwalls", {}).items():
        try:
            password = site_config.get("password") or "unused-direct-v1r"
            transport = TEDAPIv1r(
                host=site_config["host"],
                password=password,
                rsa_key_path=site_config.get("rsa_key_path", ".pypowerwall-auth/tedapi_rsa_private.pem"),
                timeout=int(site_config.get("timeout", 10)),
            )
            client = make_seeded_client(
                site_config["host"],
                site_config["din"],
                site_config.get("rsa_key_path", ".pypowerwall-auth/tedapi_rsa_private.pem"),
                password,
            )
